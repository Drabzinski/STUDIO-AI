...
const AISelectView: React.FC<{ onSelect: (ai: AIModel) => void, onBack: () => void, theme: string }> = ({ onSelect, onBack, theme }) => (
  <motion.section initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={springConfig} className="w-full max-w-5xl">
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
      {AI_OPTIONS.map(ai => (
        <button key={ai.name} onClick={() => onSelect(ai.name as AIModel)} className="group relative p-8 rounded-[2.5rem] border border-white/5 bg-white/[0.02] text-left transition-all hover:border-cyan-500/50 hover:bg-cyan-500/5 overflow-hidden">
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 2 && (
            <motion.div key="s2" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex flex-col gap-8">
              ...
              <textarea autoFocus className="w-full h-48 p-8 rounded-[2rem] border-2 bg-transparent border-white/10 outline-none focus:border-cyan-500 text-xl resize-none placeholder:opacity-20" placeholder="Ex: Escrever um e-mail de vendas..." onChange={(e) => setData({...data, objective: e.target.value})} />
          ...
          {step === 3 && (
            <motion.div key="s3" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex flex-col gap-8">
              ...
              <input autoFocus className="w-full p-8 rounded-[2rem] border-2 bg-transparent border-white/10 outline-none focus:border-cyan-500 text-xl placeholder:opacity-20" placeholder="Ex: Empreendedores de 20 a 35 anos..." onChange={(e) => setData({...data, audience: e.target.value})} />
    ...
    <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-3xl glass p-12 rounded-[3rem]">
      ...
      {step === 1 ? (
        <div className="flex flex-col gap-8">
          ...
          <textarea autoFocus className="w-full h-48 p-8 rounded-[2rem] border-2 bg-transparent border-white/10 outline-none focus:border-cyan-500 text-xl resize-none placeholder:opacity-20" placeholder="Ex: Um tigre de cristal flutuando..." onChange={(e) => setData({...data, subject: e.target.value})} />
...
const CourseView: React.FC<{ onBack: () => void, theme: string }> = ({ onBack }) => {
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        <div className="lg:w-1/3 flex flex-row lg:flex-col gap-3 overflow-x-auto pb-6 scrollbar-hide">
          {COURSE_MODULES.map(m => (
            <button key={m.id} onClick={() => setActive(m.id)} className={`p-6 rounded-[1.5rem] text-left border-2 flex items-center gap-5 shrink-0 min-w-[280px] lg:min-w-0 transition-all ${active === m.id ? 'border-cyan-500 bg-cyan-500/5 shadow-[0_0_30px_rgba(0,242,255,0.1)]' : 'border-white/5 opacity-40 hover:opacity-100'}`}>
              ...
              <div><div className="text-[10px] font-black uppercase text-cyan-500 tracking-tighter mb-1">Módulo 0{m.id}</div><div className="font-black text-sm">{m.title}</div></div>
            </button>
          ))}
        </div>
        <div className="lg:w-2/3 glass p-12 rounded-[3.5rem] shadow-2xl relative border-white/10 min-h-[500px]">
          ...
          <div className="space-y-8">
            {COURSE_MODULES.find(m => m.id === active)?.content.map((c, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} className="flex gap-6 font-medium text-lg opacity-70 leading-relaxed">
                <div className="mt-2.5 w-3 h-3 rounded-full bg-cyan-500 shrink-0 shadow-[0_0_10px_rgba(0,242,255,0.5)]" />{c}
              </motion.div>
            ))}
          </div>
          <button onClick={() => active < COURSE_MODULES.length ? setActive(active + 1) : onBack()} className="mt-16 w-full py-6 rounded-[1.5rem] bg-white text-black font-black uppercase text-xs tracking-widest hover:scale-[1.02] active:scale-95 transition-all shadow-xl">
...
const TemplatesView: React.FC<{ onCopy: (p: string) => void, copied: boolean, onBack: () => void, theme: string }> = ({ onCopy, copied, onBack }) => {
  const [filter, setFilter] = useState<'all' | 'text' | 'image'>('all');
  const [search, setSearch] = useState('');
  const filtered = useMemo(() => TEMPLATES.filter(t => (filter === 'all' || t.type === filter) && (t.title.toLowerCase().includes(search.toLowerCase()) || t.preview.toLowerCase().includes(search.toLowerCase()))), [filter, search]);
  return (
    <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full max-w-7xl">
      <div className="mb-12 flex flex-col md:flex-row items-center justify-between gap-8">
        ...
        <div className="flex flex-col md:flex-row gap-5 w-full md:w-auto">
          <div className="relative group">
            ...
            <input type="text" placeholder="Buscar 100+ templates..." className="w-full md:w-80 pl-16 pr-6 py-4 rounded-[1.5rem] border-2 bg-white/[0.03] border-white/5 outline-none focus:border-cyan-500 transition-all font-medium" value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
          <div className="p-1.5 rounded-[1.5rem] flex border-2 border-white/5 bg-white/[0.02]">
            {['all', 'text', 'image'].map((f) => (
              <button key={f} onClick={() => setFilter(f as any)} className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${filter === f ? 'bg-gradient-premium text-white shadow-lg' : 'opacity-30 hover:opacity-100'}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <AnimatePresence mode="popLayout">
          {filtered.map(t => (
            <motion.div key={t.id} layout initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="group p-8 rounded-[2.5rem] border border-white/5 bg-white/[0.02] flex flex-col justify-between gap-8 hover:border-cyan-500/30 transition-all relative overflow-hidden">
              <div>
                <div className={`w-12 h-12 rounded-2xl mb-6 flex items-center justify-center text-white ${t.type === 'text' ? 'bg-cyan-500' : 'bg-purple-600'}`}>
              ...
              <button onClick={() => onCopy(t.prompt)} className="w-full py-4 rounded-[1.2rem] border-2 border-white/5 group-hover:border-cyan-500 group-hover:bg-cyan-500 group-hover:text-white transition-all text-[11px] font-black uppercase tracking-widest">
...
const ExamplesView: React.FC<{ onBack: () => void, theme: string }> = ({ onBack }) => (
    ...
    <div className="space-y-12">
      ...
      {EXAMPLES.map(ex => (
        <div key={ex.id} className="glass p-12 rounded-[3.5rem] relative group border-white/10 hover:border-cyan-500/20 transition-all overflow-hidden">
          <h4 className="text-3xl font-black mb-12 flex items-center gap-5"><div className="w-3 h-12 bg-gradient-premium rounded-full" /> {ex.title}</h4>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 relative z-10">
            ...
            <div className="space-y-4 relative group/item">
              ...
              <div className="p-8 rounded-[2rem] border-2 border-cyan-500/50 bg-cyan-500/5 font-bold text-xl italic">"{ex.after}"</div>
              <div className="absolute -top-6 -right-4 px-6 py-3 bg-gradient-premium rounded-2xl text-[10px] font-black text-white shadow-2xl rotate-3">{ex.improvement}</div>
            </div>
          </div>
        </div>
      ))}
