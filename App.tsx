...
import { 
  ...
  ArrowLeft, Copy, RotateCcw, Check, Menu, X, Info, Zap, Compass, Target, Layers, Eye, Repeat, Trophy, Shield
} from 'lucide-react';
...
// Otimização de animações para mobile: transições mais curtas e simples
const mobileTransition = { type: "spring", damping: 25, stiffness: 200 };
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Fix para altura do Safari no iPhone
    const setHeight = () => {
  ...
  const copyToClipboard = (text: string) => {
    if (navigator.clipboard) {
      ...
      // Fallback para navegadores antigos/Safari sem HTTPS
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div 
            ...
            className={`lg:hidden border-b overflow-hidden absolute w-full left-0 z-[90] ${theme === 'dark' ? 'bg-[#0a0a0a] border-white/10' : 'bg-white border-black/5'}`}
          >
  ...
  return (
    <div className={`min-h-[100dvh] flex flex-col transition-colors duration-500 overflow-x-hidden ${theme === 'dark' ? 'bg-[#0a0a0a] text-white' : 'bg-zinc-50 text-zinc-900'}`}>
      ...
      {/* Background simplificado para Mobile (Performance Safari) */}
...
const HeroView: React.FC<{ onStart: (t: 'text' | 'image') => void }> = ({ onStart }) => (
  <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={mobileTransition} className="max-w-6xl w-full px-6 text-center py-8">
    <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-cyan-500/20 bg-cyan-500/5 text-cyan-500 text-[10px] font-black uppercase tracking-[0.2em] mb-8">
      <Sparkles size={12} /> Studio IA Profissional
    </div>
    ...
    <p className="text-sm md:text-xl mb-12 max-w-xl mx-auto opacity-60 leading-relaxed font-medium">
      Engenharia de prompts de alto nível, testada para modelos como ChatGPT e Midjourney em 2026.
    </p>
...
const AISelectView: React.FC<{ onSelect: (ai: AIModel) => void, onBack: () => void, theme: string }> = ({ onSelect, onBack, theme }) => (
  <motion.section initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={mobileTransition} className="w-full max-w-4xl px-6">
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6">
      {AI_OPTIONS.map(ai => (
        <button key={ai.name} onClick={() => onSelect(ai.name as AIModel)} className={`p-5 md:p-8 rounded-[2rem] border-2 text-left transition-all active:scale-95 flex flex-col gap-4 ${theme === 'dark' ? 'bg-white/5 border-white/5' : 'bg-white border-black/5 shadow-sm'}`}>
          <div className={`w-12 h-12 rounded-2xl ${ai.color} flex items-center justify-center text-2xl shadow-lg`}>{ai.icon}</div>
          <span className="text-base font-bold">{ai.name}</span>
        </button>
      ))}
    </div>
  </motion.section>
        ...
        {step === 1 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {TEXT_CATEGORIES.map(c => (
              <button key={c.id} onClick={() => { setData({...data, category: c.name}); setStep(2); }} className={`p-5 rounded-2xl border-2 text-left flex items-center gap-4 active:scale-95 transition-transform ${theme === 'dark' ? 'bg-white/5 border-white/5' : 'bg-white border-black/5 shadow-sm'}`}>
    ...
    <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full max-w-2xl px-6">
      ...
      {step === 1 && (
        <div className="flex flex-col gap-6">
          ...
          <textarea autoFocus className="w-full h-40 p-6 rounded-2xl border-2 bg-transparent border-white/10 outline-none focus:border-cyan-500 text-lg resize-none" placeholder="Ex: Um astronauta em Marte bebendo café..." onChange={(e) => setData({...data, subject: e.target.value})} />
...
const ResultView: React.FC<{ promptState: PromptState, onCopy: (p: string) => void, copied: boolean, onRestart: () => void, theme: string }> = ({ promptState, onCopy, copied, onRestart, theme }) => (
  <motion.section initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-3xl px-6">
    <div className="text-center mb-8">
      <h2 className="text-3xl font-black">Prompt Pronto ✨</h2>
    </div>
  ...
  return (
    <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full max-w-5xl px-6">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className={`lg:w-2/3 p-8 rounded-[2.5rem] border ${theme === 'dark' ? 'bg-white/5 border-white/10' : 'bg-white border-black/5'}`}>
          ...
          <button onClick={() => active < COURSE_MODULES.length ? setActive(active + 1) : onBack()} className="mt-12 w-full py-4 rounded-xl bg-white text-black font-black uppercase text-xs">Continuar</button>
        </div>
      </div>
    </motion.section>
  );
...
const TemplatesView: React.FC<{ onCopy: (p: string) => void, copied: boolean, onBack: () => void, theme: string }> = ({ onCopy, copied, onBack, theme }) => (
  <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full max-w-7xl px-6">
    <button onClick={onBack} className="mb-8 flex items-center gap-2 opacity-40 uppercase text-[10px] font-black"><ArrowLeft size={14} /> Home</button>
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {TEMPLATES.map(t => (
        <div key={t.id} className={`p-6 rounded-3xl border-2 flex flex-col justify-between gap-6 ${theme === 'dark' ? 'bg-white/5 border-white/5' : 'bg-white border-black/5'}`}>
          <div>
            ...
            <h4 className="font-black text-lg mb-1">{t.title}</h4>
        ...
        <div key={ex.id} className={`p-8 rounded-[3rem] border shadow-xl relative ${theme === 'dark' ? 'bg-white/5 border-white/10' : 'bg-white border-black/5'}`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="space-y-3 opacity-40">
              <span className="text-[10px] font-black uppercase text-rose-500">Pedido Comum</span>
              <div className="p-5 rounded-2xl border border-white/5 italic text-sm">"{ex.before}"</div>
            </div>
            <div className="space-y-3 relative">
              <span className="text-[10px] font-black uppercase text-cyan-500">Studio AI</span>
